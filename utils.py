...
def preprocessing_data(data, date, prediction=False):
    ...
    if prediction == False:
        filter_day = data['Transaction_date'].max() - relativedelta(
            months=date)
        ...
        filter_day = data['Transaction_date'].min() + relativedelta(
            months=date)
        filter_train = data['Transaction_date'] >= filter_day
    ...
    customer_df['spends_mean_time'] = customer_df['mean_basket'] / customer_df[
        'mean_btv']
    customer_df[
        'spends_std_time'] = customer_df['std_basket'] / customer_df['std_btv']
    customer_df['purchase_mean'] = customer_df['last_purchase'] / customer_df[
        'mean_btv']

    # Store entropy

    store_entropy = data.groupby(['Customer_no',
                                  'Store_number']).Basket_id.nunique()
    store_entropy = store_entropy.groupby('Customer_no').apply(
        lambda x: entropy(x / sum(x))).reset_index()
    store_entropy = store_entropy.rename(columns={'Basket_id': 'store_entr'})

    # Month entropy

    month_entropy = data.groupby(
        ['Customer_no', 'Transaction_date_month']).Basket_id.nunique()
    month_entropy = month_entropy.groupby('Customer_no').apply(
        lambda x: entropy(x / sum(x))).reset_index()
    month_entropy = month_entropy.rename(columns={'Basket_id': 'month_entr'})

    # Day entropy

    day_entropy = data.groupby(['Customer_no',
                                'dayofWeek']).Basket_id.nunique()
    day_entropy = day_entropy.groupby('Customer_no').apply(
        lambda x: entropy(x / sum(x))).reset_index()
    ...
    filter_month = data['Transaction_date'] >= (
        data['Transaction_date'].max() - one_month)

    last_am = data[filter_month].groupby(
        'Customer_no').item_net_amount.sum().reset_index()
    ...
    last_ev = data[filter_month].groupby(
        'Customer_no').Basket_id.nunique().reset_index()
...
def run_xgb(train_X, train_y, val_X, val_y, test_X, test_y):
    ...
    xgb_pred_y = model_xgb.predict(
        dtest, ntree_limit=model_xgb.best_ntree_limit)
...
def random_forest_importances(df, target):
    ...
    important_features = pd.Series(
        data=clf.feature_importances_, index=X.columns)
